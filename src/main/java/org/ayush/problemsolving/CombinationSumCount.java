...
Output: 2, there are two sets: [[2,2,3],[7]]
Explanation:
...
Output: 2, there are three sets [[2,2,2,2],[2,3,3],[3,5]]
Example 3:
...
public class CombinationSumCount {
    ...
    public static int combinationSum(int[] candidates, int target) {
        return getSubsets(0, new ArrayList<>(), candidates, target);
    }

    public static int getSubsets(int i, List<Integer> sublist, int[] nums, int target){
        ...
        sublist.add(nums[i]);
        int l = getSubsets(i, sublist, nums, target - nums[i]);
        ...
        int r = getSubsets(i+1, sublist, nums, target);
        return l+r;
    }
}
