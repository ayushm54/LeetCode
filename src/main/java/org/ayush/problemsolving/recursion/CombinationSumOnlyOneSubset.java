...
Output: either of [2,2,2,2], [2,3,3] or [3,5]
...
1 <= target <= 40
* */
public class CombinationSumOnlyOneSubset {
    ...
    public static boolean getSubsets(int i, List<List<Integer>> subsets, List<Integer> sublist, int[] nums, int target){
        ...
        sublist.add(nums[i]);
        if(getSubsets(i, subsets, sublist, nums, target - nums[i])) return true;
        ...
        if(getSubsets(i+1, subsets, sublist, nums, target)) return true;
        return false;
    }
}
