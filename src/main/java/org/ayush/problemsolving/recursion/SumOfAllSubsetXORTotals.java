...
public class SumOfAllSubsetXORTotals {
    ...
    public static int subsetXORSum(int[] nums) {
        ...
        return getSubsetXor(nums, 0, new ArrayList<>(), 0);
    }

    private static int getSubsetXor(int[] nums, int idx, List<Integer> sublist, int xorSum) {
        if (idx >= nums.length) {
            if(sublist.size()>0) {
        ...
        xorSum = getSubsetXor(nums, idx+1, sublist, xorSum);
        ...
        sublist.remove(Integer.valueOf(nums[idx]));
        xorSum = getSubsetXor(nums, idx+1, sublist, xorSum);
        return xorSum;
    }
}
