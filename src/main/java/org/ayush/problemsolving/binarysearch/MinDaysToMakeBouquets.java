...
public class MinDaysToMakeBouquets {
    ...
    public static int minDays(int[] bloomDay, int m, int k) {
        ...
        if(m*k > n) return -1;
        int[] maxMin = getMax(bloomDay);
        // the answer will always lie between the min days ands max days to bloom a flower
        int ans = -1;
        int low = maxMin[1], high = maxMin[0];
        while(low <= high) {
            int mid = low + (high - low) / 2;
            int count = getBouquetsCount(mid, bloomDay, k);
            if(count >= m){
                ans = mid;
                high = mid - 1;
            }else{
    ...
    private static int getBouquetsCount(int mid, int[] bloomDay, int k) {
        int count = 0;
        int ans = 0;
