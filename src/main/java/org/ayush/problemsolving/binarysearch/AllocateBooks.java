...
public class AllocateBooks {
    public static void main(String[] args) {
        System.out.println(findPages(new ArrayList<>(Arrays.asList(new Integer[]{12, 34, 67, 90})), 4, 2));
        System.out.println(findPages(new ArrayList<>(Arrays.asList(new Integer[]{25, 46, 28, 49, 24})), 5, 4));
    }

    public static int findPages(ArrayList<Integer> arr, int n, int m) {
        if(m > n) return -1;
        int[] maxAndTotal = getMaxAndTotal(arr);
        int low = maxAndTotal[0], high = maxAndTotal[1];
        int ans = Integer.MAX_VALUE;
        while (low <= high) {
            int mid = low + (high - low) / 2;
            int studentCount = countStudents(arr, m, mid);
            if(studentCount <= m){
    ...
    private static int countStudents(ArrayList<Integer> arr, int m, int max) {
