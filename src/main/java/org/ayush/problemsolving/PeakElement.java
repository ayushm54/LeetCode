...
public class PeakElement {
    ...
    private static int findPeakElement(int[] arr) {
        ...
        if(arr[low] > arr[low+1]) return low; // to handle cases were tha array is strictly decreasing
        if(arr[high] > arr[high-1]) return high; // to handle cases were tha array is strictly increasing
        // we will trim the search space becuase we have explicit checks for 1st and last element
        low = low + 1;
        high = high - 1;
        while(low <= high) {
